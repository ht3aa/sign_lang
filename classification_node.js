const CLASSES = 34;
const MODEL_HANDS_INPUT_LENGTH = 1890;
const tf = require("@tensorflow/tfjs-node");
const fs = require("fs");
// const summaryWriter = tf.node.summaryFileWriter('./tenosorBoard');
const train = async (modelLength) => {
  fs.readFile("./data_34_hasan.csv", "utf8", async (err, data) => {
    if (err) {
      console.error(err);
      return;
    }
    ...
    let result = await model.fit(INPUT_TENSOR, OUTPUT_TENSOR, {
      ...
      batchSize: 34,
      epochs: 70,
      callbacks: tf.node.tensorBoard("./tensorBoard"),
    });
